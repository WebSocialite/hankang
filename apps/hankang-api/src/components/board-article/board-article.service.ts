...
export class BoardArticleService {
    ...
    public async getBoardArticle(memberId: ObjectId, articleId: ObjectId): Promise<BoardArticle> {
        const search: T = {
            _id: articleId,
            articleStatus: BoardArticleStatus.ACTIVE, // faqat ACTIVE ni koraoladi
        };
        ...
        if (newView) {  // agar birinchi marta korayotgan bolsa view soni oshadi
   ...
   public async likeTargetBoardArticle(memberId: ObjectId, likeRefId: ObjectId): Promise<BoardArticle> {
    ...
    public async getAllBoardArticlesByAdmin (input: AllBoardArticlesInquiry): Promise<BoardArticles> {
        ...
        const result = await this.boardArticleModel
        .aggregate([
            { $match: match },  // pipelines
            { $sort: sort },
            {
                $facet: {
                    list: [  // pipelines
                        { $skip: ( input.page - 1) * input.limit }, //pipelines
                        { $limit: input.limit },
                        // me Liked 
                        lookupMember,
                        { $unwind: '$memberData'}, // unwind arrayni ichidagi datani olib beradi Arrayni esa tashlavoradi
